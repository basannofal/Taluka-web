...
function Panchayat() {
    ...
    const deleteData =async (stfid) => {
        const res7 = await fetch(`/deletepanchayat/${stfid}/${id}`, {
            method: "GET",
            headers: {
              "Content-Type": "application/json"
            },
          ...
          if (!data4) {
            ...
            navigate(`/panchayat/${id}`, { replace: true })
            ...
            <div class="sales-boxe">
                <div class="recent-sales box">
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        <div class="title">Panchayat Information</div>
                        <div><NavLink to={`/addpanchayat/${id}`} > <a><button className='btn btn-primary'>Add Panchayat <i class='bx bx-plus'></i></button> </a></NavLink> </div>
                    </div>
                    <hr />
                        ...
                        <table class="table table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">Panchayat Address</th>
                                    <th scope="col">Sarpanch Name</th>
                                    <th scope="col">Sarpanch Number</th>
                                    <th scope="col">Village Code</th>
                                    <th scope="col">Total Gents</th>
                                    <th scope="col">Total Ladies</th>
                               ...
                               
                        {
                            userdata.map((ele, id) => {
                                return (<>
                                    <tr>
                                        <td>{id + 1}</td>
                                        <td>{ele.villagename}</td>
                                        <td>{ele.panchayataddress}</td>
                                        <td>{ele.sarpanchname}</td>
                                        <td>{ele.sarpanchnumber}</td>
                                        <td>{ele.villagecode}</td>
                                        <td>{ele.totalpeople}</td>
                                        <td>{ele.peopleladies}</td>
                                        <td>
