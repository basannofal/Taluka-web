...
function Notice() {
    ...
    const deleteData =async (stfid) => {
        const res7 = await fetch(`/deletenotice/${stfid}/${id}`, {
            method: "GET",
            headers: {
              "Content-Type": "application/json"
            },
            ...
            <div class="sales-boxe">
                <div class="recent-sales box">
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        <div><NavLink to={`/addnotice/${id}`} > <a><button className='btn btn-primary'>Add Notice <i class='bx bx-plus'></i></button> </a></NavLink> </div>
                    </div>
                    <hr />
                            ...
                            userdata.map((ele, id) => {
                                return (<>
                                    <tr>
                                        <td>{id + 1}</td>
                                        <td>{ele.noticemsg}</td>
                                        <td className='text-center'>{ele.noticedate}</td>
                                        <td>
